...
export default function ActivityList({ activities, dispatch } : ActivityListProps) {
    ...
    const acticitiesIsEmpty = useMemo(() => activities.length === 0, [activities])
    return (
        <>
            <h2 className="text-4xl font-bold text-slate-600 text-center">Comida y Actividades</h2> 

            {acticitiesIsEmpty ? <p className="text-center">No háy actividades...</p> :
                activities.map(activity => (
                    <div key={activity.id} className="px-5 py-10 mt-5 flex justify-between shadow-lg">
